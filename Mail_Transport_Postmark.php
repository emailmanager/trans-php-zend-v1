<?php
/**
 * Emailmanager Mail Transport Class for Zend_Mail
 *
 * Copyright 2010, Alistair Phillips, http://the.0gravity.co.uk/universe/php/zend/mail_transport_emailmanager/
 *
...
class Mail_Transport_Emailmanager extends Zend_Mail_Transport_Abstract
{
    /**
     * API key required by Emailmanager
     *
     * @var string
     */
    private $_apiKey = '';
    ...
    public function _sendMail()
    {
	    ...
	    $tags = array();
        if (array_key_exists('emailmanager-tag', $headers)) {
            reset($headers['emailmanager-tag']);
            foreach ($headers['emailmanager-tag'] as $key => $val) {
                if (empty($key) || $key != 'append')
                {
                    $tags[] = $val;
                }
        ...
        $client->setUri( 'http://trans.emailmanager.com/email' );
        ...
        if ( $response->getStatus() != 200 ) {
            throw new Exception( 'Mail not sent - Emailmanager returned ' . $response->getStatus() . ' - ' . $response->getMessage() );
        }
    }
}
